...
def test_add_record(chromadb_store):
    ...
    assert record_from_db.document == 'Hello world'
...
def test_add_bundle(chromadb_store):
    bundle = [
        Record(
            id='test2',
            document="What is past is prologue.",
            metadata={'source': 'The Tempest'}
        ),
        Record(
            id='test3',
            document="It is the east, and Juliet the sun.",
            # metadata={},
        ),
        Record(
            id='test4',
            document="I knew him, Horatio",
            metadata={'source': 'Hamlet'}
        ),
        Record(
            id='test5',
            document="Neither a borrower, nor a lender be.",
            # metadata={},
        ),
    ]
    ...
    assert after_count - before_count == len(bundle)
    assert test2.id and test2.document and test2.metadata and not (test2.image or test2.uri or test2.embedding)
    assert test3.id and test3.document and not (test3.image or test3.uri or test3.embedding or test3.metadata)
    assert test4.id and test4.document and test4.metadata and not (test4.image or test4.uri or test4.embedding)
    assert test5.id and test5.document and not (test5.image or test5.uri or test5.embedding or test5.metadata)
    assert test2.id == "test2" and test3.id == "test3" and test4.id == "test4" and test5.id == "test5"
    assert test3.document == "It is the east, and Juliet the sun."
...
def test_single_query(chromadb_store):
    ...
    bundle = [
        Record(
            id="spaghetti",
            document="""Spaghetti (Italian: [spagetti]) is a long, thin, solid, cylindrical pasta. It is a staple of traditional Italian cuisine.
        ...
        Record(
            id="new_south_wales",
            document="""New South Wales (commonly abbreviated as NSW) is a state on the east coast of Australia.
        ...
        Record(
            id="italy",
            document="""Italy, officially the Italian Republic, is a country in Southern and Western Europe.
...
def test_query_limit(chromadb_store):
    bundle = [
        Record(id=f"record{i}", document=f"document {i}")
        for i in range(50)
    ]
